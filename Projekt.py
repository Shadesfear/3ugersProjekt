...
for x in range(1,8,1):
	plt.figure(x, figsize=(3.5, 2.8))
	plt.plot(table['forsoeg456'][:-(wi-1)*Na,0],movingaveragex(arb['forsoeg'+str(x)+'20'],wi,Na),"s",table['forsoeg456'][:-(wi-1)*Na,0],movingaveragex(arb['forsoeg'+str(x)+'40'],wi,Na),"*",table['forsoeg456'][:-(wi-1)*Na,0],movingaveragex(arb['forsoeg'+str(x)+'60'],wi,Na),"x",table['forsoeg456'][:-(wi-1)*Na,0],movingaveragex(arb['forsoeg'+str(x)+'80'],wi,Na),"o", markersize = 1.1)
	plt.xlabel('Wavelength [nm]', fontsize = 8)
	plt.ylabel('Intensity [n]', fontsize = 8)
	plt.xticks(fontsize = 6)
	plt.yticks(fontsize = 6, rotation = 'vertical')
	plt.tight_layout()
	plt.savefig('figure'+str(x)+'.png',dpi=600)
